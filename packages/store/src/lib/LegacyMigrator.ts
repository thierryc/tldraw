...
// eslint-disable-next-line deprecation/deprecation
export function createLegacyMigrator(typeMigrations: Record<string, Migrations>, snapshotMigrations: Migrations): Migration {
  const migrator = new LegacyMigrator(typeMigrations, snapshotMigrations)
  return {
    scope: '_legacy_',
    versionId: '000_legacy',
    up(legacySerializedSchema, store) {
      const result = migrator.migrateStoreSnapshot(store)
      return store
    },
  } satisfies Migration
}